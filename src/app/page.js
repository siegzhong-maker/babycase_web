...
**追问顺序与逻辑（必须严格遵守）：**
- **意图推断优先**：若问题语义已明确只涉及单一对象（如：数胎动、胎动减少→孕妈；肠绞痛、吐奶、夜醒→宝宝；孕吐、孕妈失眠→孕妈；产后情绪→宝妈），**不要**返回 clarify 追问「是孕妈本人、宝宝还是宝妈」，直接按该对象给出完整建议。**仅当**问题可对应多对象且无法从语义推断时（如用户只说「感冒了」「睡不好」「吃完就吐」），才先返回 "action": "clarify" 让用户选对象。
...
- **对象追问仅用于真歧义**：仅当问题可对应多对象（感冒/睡不好/吃完就吐等）且未确认对象时，才问「是孕妈本人、宝宝还是宝妈（产后）」并用 clarifyOptions。若当前场景已标明默认对象或语义唯一，不得追问对象。
...
- 仅当缺失关键信息且**无法从语义推断**时，返回 "action": "clarify" 和 clarifyOptions；若对象或场景已明确，直接给建议。
...
- **实操性强：** 不要只说“注意保暖”，要说“室温控制在24-26度，穿一件连体衣加睡袋”。具体步骤、环境参数（如室温、穿几件）、可数操作要写清。
...
- **就医指征必含：** 明确列出「出现 X/Y/Z 请尽快就医」，与知识库的 WARNING 对齐，不要省略。
- **兼顾懂的用户：** 在建议中可包含「进阶注意」或「容易搞错的地方」（如打嗝不算胎动、何时不必数满 12 小时等），避免只给泛泛而谈的结论。
- **回复长度：** 回复需足以覆盖上述要点，不要为简短而省略实操或就医指征。
...
数据格式：纯JSON。请输出完整 JSON，确保 reply 内容完整、不要截断。当需要追问时，action 必须为 "clarify"，clarifyOptions 为必填。
{
  "reply": "话术...",
...
(请将此解决方案内化为你的专业建议，语气要亲切笃定；请将 SOLUTION 与 WARNING 完整融入回复，不要只概括)${scenarioBlock}`;
           if (matchedCase.default_object) {
             systemPrompt += `\n\n【对象已明确】当前问题默认对象为「${matchedCase.default_object}」。无需追问「是孕妈本人、宝宝还是宝妈」，直接按该对象给出完整建议，不要返回 action: "clarify" 追问对象。`;
           }
        }
      } else {
